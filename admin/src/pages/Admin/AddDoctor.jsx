...
const AddDoctor = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
      <p className='mb-5 text-xl font-medium'>Add Doctor</p>
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        <div className='flex items-center gap-4 mb-8 text-gray-500'>
          <label htmlFor="doc-img">
            <img className='w-16 bg-purple-300 rounded-full cursor-pointer' src={docImg ? URL.createObjectURL(docImg) :assets.upload_area} alt="" />
          </label>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
          <div className='w-full lh:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex-1 flex flex-col gap-1'>
              <p>Fees</p>
              <input onChange={(e)=> setFees(e.target.value)} value={fees} className='border rounded px-3 py-1 '  type="number" placeholder='Your Fees' required />
            </div>
        ...
        <div>
          ...
          <textarea onChange={(e)=> setAbout(e.target.value)} value={about} className='w-full px-4 pt-2 rounded border' placeholder='Write About Doctor' rows={5} required />
        </div>
        <br/>

        <button type='submit' className='bg-primary px-10 py-2  text-white rounded-full'>Add Doctor</button>
